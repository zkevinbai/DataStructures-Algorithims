...
function reverseLinkedList(linkedList) {
  ...
  let trimmedHead = (linkedList) => {
    let removed = linkedList.head;
    linkedList.head = removed.next;
    linkedList.length -= 1;
    return removed;
  }

  let head = trimmedHead(linkedList);

  return reverseLinkedList(linkedList).addToTail(head.value);
}
